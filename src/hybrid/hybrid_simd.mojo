...
struct HybridSIMD[type: DType, size: Int, square: FloatLiteral](StringableCollectionElement, Formattable, EqualityComparable):
    """
    ...
    fn cast[target_type: DType](self) -> HybridSIMD[target_type, size, square]:
        ...
        return HybridSIMD[target_type, size, square](self.re.cast[target_type](), self.im.cast[target_type]())
    ...
    fn get_antiox[_square: FloatLiteral](self) -> Self.Coef:
        constrained[sign(square) == sign(_square), "cannot get antiox: the squares signs must match"]()
        return self.im * (Self.unital_factor / ufac(_square))

    @always_inline
    fn set_antiox[_square: FloatLiteral](inout self, antiox: Self.Coef):
        constrained[sign(square) == sign(_square), "cannot set antiox: the squares signs must match"]()
    ...
    fn fma[__:None=None](self, mul: Self, acc: Self.Coef) -> Self:
        """Fused multiply add."""
        return HybridSIMD[type,size,square](self.re.fma(mul.re, self.im.fma(square*mul.im, acc)), self.re.fma(mul.im, self.im*mul.re))
    ...
    fn fma[__:None=None, ___:None=None](self, mul: Self, acc: Self) -> Self:
        """Fused multiply add."""
        return HybridSIMD[type,size,square](self.re.fma(mul.re, self.im.fma(square*mul.im, acc.re)), self.re.fma(mul.im, self.im.fma(mul.re, acc.im)))

    @always_inline
    fn shuffle[*mask: Int](self) -> Self:
        """Shuffle elements along the SIMD axis using a permutation mask."""
        return Self(self.re._shuffle_list[mask](self.re), self.im._shuffle_list[mask](self.im))

    @always_inline
    fn shuffle[*mask: Int](self, other: Self) -> Self:
        """Shuffle elements along the SIMD axis using a permutation mask and add to other."""
        return Self(self.re._shuffle_list[mask](other.re), self.im._shuffle_list[mask](other.im))
    ...
    fn slice[slice_size: Int, offset: Int](self) -> HybridSIMD[type,slice_size,square]:
        ...
        return HybridSIMD[type,slice_size,square](self.re.slice[slice_size, offset=offset](), self.im.slice[slice_size, offset=offset]())
    ...
    fn interleave(self, other: Self) -> HybridSIMD[type,2*size,square]:
        ...
        return HybridSIMD[type,2*size,square](self.re.interleave(other.re), self.im.interleave(other.im))

    @always_inline
    fn deinterleave(self) -> (HybridSIMD[type, size//2, square], HybridSIMD[type, size//2, square]):
        """Deinterleaves this HybridSIMD vector into even and odd indicies."""
        var re = self.re.deinterleave()
        var im = self.im.deinterleave()
        return (HybridSIMD[type, size//2, square](re[0], im[0]), HybridSIMD[type, size//2, square](re[1], im[1]))

    @always_inline
    fn reduce[func: fn(a: HybridSIMD, b: __type_of(a)) capturing -> __type_of(a), size_out: Int = 1](self) -> HybridSIMD[type,size_out,square]:
    ...
    fn measure[absolute: Bool = False](self) -> Self.Coef:
        ...
        """
        @parameter
        if square == 0: return abs(self.re)
    ...
    @always_inline # Hybrid + Hybrid
    fn __add__[__:None=None](self, other: HybridSIMD[type, size, _]) -> MultiplexSIMD[type,size]:
    ...
    @always_inline # Hybrid - Hybrid
    fn __sub__[__:None=None](self, other: HybridSIMD[type, size, _]) -> MultiplexSIMD[type,size]:
        return MultiplexSIMD(self) - other
    ...
    fn __mul__[__:None=None](self, other: Self) -> Self:
        return Self(self.re*other.re + square*self.im*other.im, self.re*other.im + self.im*other.re)
    ...
    fn __pow__(self, other: Self.Coef) -> Self:
        if other == 1:
            ...
            @parameter
            if square == 0:
                return pow(self.re, other-1) * HybridSIMD[type,size,square](self.re, self.im*other)
            else:
    ...
    @always_inline # Hybrid + Hybrid
    fn __radd__[__:None=None](self, other: HybridSIMD[type, size, _]) -> MultiplexSIMD[type,size]:
        return other + self
    ...
    @always_inline # Hybrid - Hybrid
    fn __rsub__[__:None=None](self, other: HybridSIMD[type, size, _]) -> MultiplexSIMD[type,size]:
        return other - self
